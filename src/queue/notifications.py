...
class EmailNotifier(NotificationProvider):
    ...
    async def send(self, recipient: str, subject: str, body: str, metadata: Optional[Dict] = None) -> bool:
        if not self.host:
            ...
            return False
        # ponytail: blocking smtplib moved off the event loop; still synchronous per send
        return await asyncio.to_thread(self._send_sync, recipient, subject, body)

    def _send_sync(self, recipient: str, subject: str, body: str) -> bool:
        try:
...
class NotificationDispatcher:
    ...
    async def send(
        ...
        return await provider.send(recipient, subject, body, metadata)
